...
pub fn get_output_dir_name(input_dir: &PathBuf) -> Result<PathBuf, String> {
    ...
    if let None = &input_dir.file_name() {
        return Err(String::from("The last segment of the input path is not valid!"));
    };
    ...
    Ok(parent.join(format!("{}-html5picture", input_dir_name.to_str().unwrap())))
}

/// Removes the given base directory from the given input file.
pub fn remove_base_dir(base_dir: &PathBuf, input_file: &PathBuf) -> Result<PathBuf, String> {
    match pathdiff::diff_paths(input_file, base_dir) {
        ...
        None => {
            match input_file.to_str() {
                Some(v) => Err(
                    String::from(
                        format!("Could not remove base dir of {}", v)
                        )
                    ),
